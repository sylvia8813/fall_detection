...
static void dp_app_cv_mb_cls_eventhdl_cb(EVT_INDEX_E event)
{
	uint16_t err;
	int32_t read_status;
...
// #endif
	...
	if( g_trans_type == 0 )// transfer type is (UART) 
	{
		cv_mb_cls_run(&algoresult_yolov8n_ob);
	}
	else if( g_trans_type == 1 || g_trans_type == 2)// transfer type is (SPI) or (UART & SPI) 
	{
		#if TOTAL_STEP_TICK
...
void app_start_state(APP_STATE_E state)
{
	...
	{
		if(state == APP_STATE_ALLON_MB_CLS)
		{
			if(cisdp_dp_init(true, SENSORDPLIB_PATH_INT_INP_HW5X5_JPEG, dp_app_cv_mb_cls_eventhdl_cb, 4, APP_DP_RES_RGB640x480_INP_SUBSAMPLE_2X) < 0)
			{
				xprintf("\r\nDATAPATH Init fail\r\n");
				APP_BLOCK_FUNC();
			}
...
#endif
	if(g_use_case == 0) {
		xprintf("mobilenet classification\n");
#ifdef EN_ALGO
		cv_mb_cls_init(true, true, MOBILENET_CLASSIFICATION_FLASH_ADDR, MOBILENET_CLASSIFICATION_MODEL_SIZE);
#endif
#if TEST_SAMLL_MODEL || UINT_TEST
		cv_mb_cls_run(&algoresult_yolov8n_ob);
#else
	    app_start_state(APP_STATE_ALLON_MB_CLS);
#endif
	}
	return 0;
}
