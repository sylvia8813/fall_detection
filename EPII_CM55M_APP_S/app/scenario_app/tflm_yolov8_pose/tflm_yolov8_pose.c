...
// #endif
	...
	else if( g_trans_type == 1 || g_trans_type == 2)// transfer type is (SPI) or (UART & SPI) 
	{
		...
		for (int i = 0; i < MAX_TRACKED_YOLOV8_ALGO_RES; i++) {
			algoresult_yolov8_pose.dypr[i].bbox.x = 0;
			algoresult_yolov8_pose.dypr[i].bbox.y = 0;
			algoresult_yolov8_pose.dypr[i].bbox.width = 0;
			algoresult_yolov8_pose.dypr[i].bbox.height = 0;
			algoresult_yolov8_pose.dypr[i].confidence = 0;
			for(int k = 0; k < HUMAN_POSE_POINT_NUM; k++)
			{
				algoresult_yolov8_pose.dypr[i].hpr[k].x = 0;
				algoresult_yolov8_pose.dypr[i].hpr[k].y = 0;
				algoresult_yolov8_pose.dypr[i].hpr[k].score = 0;
			}
			...
			|| g_cdm_fifoerror == 1 || g_xdma_abnormal == 1 || g_hxautoi2c_error == 1)
	{
		...
		// 👇 新增以下程式碼：印出警告，清空所有錯誤狀態，並叫硬體重新抓圖 👇
		xprintf("Datapath Timeout! Retriggering...\n");
		g_inp1bitparer_abnormal = 0;
		g_wdt1_timeout = 0;
		g_wdt2_timeout = 0;
		g_wdt3_timeout = 0;
		...
		sensordplib_retrigger_capture(); // 最重要的一行：重新啟動影像管線
	}
}

void app_start_state(APP_STATE_E state)
{
	xprintf("g_trans_type = %d\n", g_trans_type);
