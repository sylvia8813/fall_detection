...
struct_yolov8_gender_cls_algoResult algoresult_yolov8_gender_cls;
...
static void dp_app_cv_yolov8n_gender_cls_eventhdl_cb(EVT_INDEX_E event)
{
	uint16_t err;
	int32_t read_status;
...
// #endif
	...
	if( g_trans_type == 0 )// transfer type is (UART) 
	{
		cv_yolov8_gender_cls_run(&algoresult_yolov8_gender_cls);
	}
	else if( g_trans_type == 1 || g_trans_type == 2)// transfer type is (SPI) or (UART & SPI) 
	{
		#if TOTAL_STEP_TICK
			...
			cv_yolov8_gender_cls_run(&algoresult_yolov8_gender_cls);
				...
				xprintf("Tick for TOTAL YOLOv8n gender cls:[%d]\r\n",(loop_cnt_2-loop_cnt_1)*CPU_CLK+(systick_1-systick_2));					
			#endif	

			if(g_trans_type == 1)//USE SPI
			{
				algoresult_yolov8_gender_cls.algo_tick = (loop_cnt_2-loop_cnt_1)*CPU_CLK+(systick_1-systick_2);		
			} 		
		#endif

		#if FRAME_CHECK_DEBUG
			hx_drv_spi_mst_protocol_write_sp((uint32_t)&algoresult_yolov8_gender_cls, sizeof(struct_yolov8_gender_cls_algoResult), DATA_TYPE_META_YOLOV8_GENDER_CLS_DATA);
		#endif
	}
#else 
	#if TOTAL_STEP_TICK
			...
			SystemGetTick(&systick_2, &loop_cnt_2);
		#if TOTAL_STEP_TICK_DBG_LOG
			xprintf("Tick for TOTAL yolov8_gender_cls:[%d]\r\n",(loop_cnt_2-loop_cnt_1)*CPU_CLK+(systick_1-systick_2));				
		#endif	
		algoresult_yolov8_gender_cls.algo_tick = (loop_cnt_2-loop_cnt_1)*CPU_CLK+(systick_1-systick_2);				
	#endif
		...
		{
			algoresult_yolov8_gender_cls.obr[i].bbox.x = 0;
			algoresult_yolov8_gender_cls.obr[i].bbox.y = 0;
			algoresult_yolov8_gender_cls.obr[i].bbox.width = 0;
			algoresult_yolov8_gender_cls.obr[i].bbox.height = 0;
			algoresult_yolov8_gender_cls.obr[i].confidence = 0;
			algoresult_yolov8_gender_cls.obr[i].class_idx = 0;

			algoresult_yolov8_gender_cls.gen_cls[i].bbox.x = 0;
			algoresult_yolov8_gender_cls.gen_cls[i].bbox.y = 0;
			algoresult_yolov8_gender_cls.gen_cls[i].bbox.width = 0;
			algoresult_yolov8_gender_cls.gen_cls[i].bbox.height = 0;
			algoresult_yolov8_gender_cls.gen_cls[i].confidence = 0;
			algoresult_yolov8_gender_cls.gen_cls[i].class_idx = 0;
...
void app_start_state(APP_STATE_E state)
{
	...
	{
		if(state == APP_STATE_ALLON_YOLOV8N_GENDER_CLS)
		{
			if(cisdp_dp_init(true, SENSORDPLIB_PATH_INT_INP_HW5X5_JPEG, dp_app_cv_yolov8n_gender_cls_eventhdl_cb, 4, APP_DP_RES_YUV640x480_INP_SUBSAMPLE_2X) < 0)
			{
				xprintf("\r\nDATAPATH Init fail\r\n");
				APP_BLOCK_FUNC();
			}
	...
	else if ( g_trans_type == 1 ) // only use SPI 
	{
		if(state == APP_STATE_ALLON_YOLOV8N_GENDER_CLS)
		{
			if(cisdp_dp_init(true, SENSORDPLIB_PATH_INT_INP_HW5X5_JPEG, dp_app_cv_yolov8n_gender_cls_eventhdl_cb, 4, APP_DP_RES_YUV640x480_INP_SUBSAMPLE_1X) < 0)
			{
				xprintf("\r\nDATAPATH Init fail\r\n");
				APP_BLOCK_FUNC();
			}
...
#endif
	if(g_use_case == 0) {
		xprintf("YOLOv8n gender classification\n");
#ifdef EN_ALGO
		cv_yolov8_gender_cls_init(true, true, FACE_DECTECT_FLASH_ADDR, YOLOV8N_GENDER_CLS_FLASH_ADDR);
#endif
	    app_start_state(APP_STATE_ALLON_YOLOV8N_GENDER_CLS);
	}
	return 0;
}
