...
static void cisdp_wdma_addr_init(APP_DP_INP_SUBSAMPLE_E subs)
{
	...
	g_wdma1_baseaddr = mm_reserve(((623+ (IMX219_HW5x5_CROP_WIDTH/16)*(IMX219_HW5x5_CROP_HEIGHT/16)* 38 + 35) >>2 ) <<2);   //YUV420 x10 Compress = ((623+ (W/16)*(H/16)* 38 + 35) >>2 ) <<2  byte
	if(g_wdma1_baseaddr!=0)
		g_wdma2_baseaddr = g_wdma1_baseaddr;
	else
		return ;

	g_wdma3_baseaddr= mm_reserve(IMX219_HW5x5_CROP_WIDTH*IMX219_HW5x5_CROP_HEIGHT*3/2);     //YUV420: Y= W*H byte, U = ((W*H)>>2) byte, V = ((W*H)>>2) byte
...
int cisdp_sensor_init()
{
    ...
    {
    	dbg_printf(DBG_LESS_INFO, "IMX219 off by app fail\r\n");
        return -1;
    }

	if(hx_drv_cis_setRegTable(IMX219_init_setting, HX_CIS_SIZE_N(IMX219_init_setting, HX_CIS_SensorSetting_t))!= HX_CIS_NO_ERROR)
	{
		dbg_printf(DBG_LESS_INFO, "IMX219 Init Stream by app fail \r\n");
		return -1;
	}
	else
	{
...
void cisdp_stream_on()
{
    ...
    {
    	dbg_printf(DBG_LESS_INFO, "IMX219 on by app fail\r\n");
        return;
    }
    else
    {
...
void cisdp_stream_off()
{
    ...
    {
    	dbg_printf(DBG_LESS_INFO, "IMX219 off by app \n");
    }
}
