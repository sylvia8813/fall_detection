...
std::string img_2_json_str_assign_buf(el_img_t* img, char* assgin_img_2_json_str_buffer) {
    if (!img || !img->data || !img->size) [[unlikely]]
    {
        return std::string("\"image\": \"\"");
    }
    ...
    std::memset(assgin_img_2_json_str_buffer, 0, buffer_size);
    el_base64_encode(img->data, img->size, assgin_img_2_json_str_buffer);
    DEV_UART* console_uart;
    console_uart = hx_drv_uart_get_dev((USE_DW_UART_E)CONSOLE_UART_ID);
    console_uart->uart_open(UART_BAUDRATE_921600);
    // std::string tmp_buf = "[image]";
    // console_uart->uart_write(tmp_buf.c_str()  , sizeof(tmp_buf) );
    console_uart->uart_write(assgin_img_2_json_str_buffer  , sizeof(char)*(buffer_size) );
    char temp[1]  = {'\n'};
    console_uart->uart_write(temp  , sizeof(char) );
    // tmp_buf = "[end image]";
    // console_uart->uart_write(tmp_buf.c_str()  , sizeof(tmp_buf) );
    // for(int i =0; i < size;i++) printf("%s",assgin_img_2_json_str_buffer[i]);
    return concat_strings("\"image\": \"", assgin_img_2_json_str_buffer, "\"");
}
...
std::string  peoplenet_box_results_2_json_str(std::forward_list<el_box_t>& results) {
    std::string ss;
    const char* delim = "";
