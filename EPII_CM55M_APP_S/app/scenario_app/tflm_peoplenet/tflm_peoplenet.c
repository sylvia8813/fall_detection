...
struct_peoplenet_algoResult algoresult_peoplenet;
...
static void dp_app_cv_peoplenet_eventhdl_cb(EVT_INDEX_E event)
{
	uint16_t err;
	int32_t read_status;
...
// #endif
	...
	if( g_trans_type == 0 )// transfer type is (UART) 
	{
		cv_peoplenet_run(&algoresult_peoplenet);
	}
	else if( g_trans_type == 1 || g_trans_type == 2)// transfer type is (SPI) or (UART & SPI) 
	{
		#if TOTAL_STEP_TICK
			...
			cv_peoplenet_run(&algoresult_peoplenet);
				...
				xprintf("Tick for TOTAL peoplenet :[%d]\r\n",(loop_cnt_2-loop_cnt_1)*CPU_CLK+(systick_1-systick_2));					
			#endif	

			if(g_trans_type == 1)//USE SPI
			{
				algoresult_peoplenet.algo_tick = (loop_cnt_2-loop_cnt_1)*CPU_CLK+(systick_1-systick_2);		
			} 		
		#endif

		#if FRAME_CHECK_DEBUG
			hx_drv_spi_mst_protocol_write_sp((uint32_t)&algoresult_peoplenet, sizeof(struct_peoplenet_algoResult), DATA_TYPE_META_PEOPLENET_DATA);
		#endif
	}
#else 
	#if TOTAL_STEP_TICK
...
#endif
		//algoresult_peoplenet
		for (int i = 0; i < MAX_TRACKED_PEOPLENET_RES; i++) {

			for(int k = 0; k < 3; k++)
			{
				algoresult_peoplenet.result[i][k].bbox.x = 0;
				algoresult_peoplenet.result[i][k].bbox.y = 0;
				algoresult_peoplenet.result[i][k].bbox.width = 0;
				algoresult_peoplenet.result[i][k].bbox.height = 0;
				algoresult_peoplenet.result[i][k].confidence = 0;
			}
...
void app_start_state(APP_STATE_E state)
{
	...
	{
		if(state == APP_STATE_PEOPLENET)
		{
			if(cisdp_dp_init(true, SENSORDPLIB_PATH_INT_INP_HW5X5_JPEG, dp_app_cv_peoplenet_eventhdl_cb, 4, APP_DP_RES_RGB640x480_INP_SUBSAMPLE_2X) < 0)
			{
				xprintf("\r\nDATAPATH Init fail\r\n");
				APP_BLOCK_FUNC();
			}
...
// #endif
	...
	if(g_use_case == 0) {
		xprintf("peoplenet\n");
#ifdef EN_ALGO
		cv_peoplenet_init(true, true, PEOPLENET_FLASH_ADDR);
#endif
	    app_start_state(APP_STATE_PEOPLENET);
	}
	return 0;
}
