...
static void cisdp_wdma_addr_init(APP_DP_INP_SUBSAMPLE_E subs)
{
	...
	g_wdma1_baseaddr = mm_reserve(((623+ (IMX708_HW5x5_CROP_WIDTH/16)*(IMX708_HW5x5_CROP_HEIGHT/16)* 38 + 35) >>2 ) <<2);   //YUV420 x10 Compress = ((623+ (W/16)*(H/16)* 38 + 35) >>2 ) <<2  byte
	if(g_wdma1_baseaddr!=0)
		g_wdma2_baseaddr = g_wdma1_baseaddr;
	else
		return ;

	g_wdma3_baseaddr= mm_reserve(IMX708_HW5x5_CROP_WIDTH*IMX708_HW5x5_CROP_HEIGHT*3/2);     //YUV420: Y= W*H byte, U = ((W*H)>>2) byte, V = ((W*H)>>2) byte
