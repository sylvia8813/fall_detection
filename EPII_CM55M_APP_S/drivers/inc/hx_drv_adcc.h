...
/**
* \defgroup   ADC-LV
* \brief      ADC-LV Modulation.
* \details    Analog-to-Digital (12bit) Converter Controller module 
* <pre>
* Usage-1: Using channel 0 to obtain ADC value, with a maximum input voltage selected as 1.8V.  
*    Sample code: 
*      uint16_t data;
*      ADCC_CFG_T g_adcc_cfg;
*      hx_drv_adcc_get_defCfg(&g_adcc_cfg);
*
*      g_adcc_cfg.analog_vol = ADCC_VOL_1P8V;
*      hx_drv_adcc_init(HW_ADCC_BASEADDR, &g_adcc_cfg);
*
*      /// unmask channel 0
*      hx_drv_adcc_set_channel_mask(~ADCC_CH0); 
*
*      /// enable ADC module
*      hx_drv_adcc_set_enable(1);
*
*      /// get channel 0 ADC data
*      hx_drv_adcc_get_sample_data(ADCC_CHANNEL0, &data);
*
*
...
*      hx_drv_adcc_set_high_threshold(ADCC_CHANNEL0, 2304);
*      hx_drv_adcc_set_low_threshold(ADCC_CHANNEL0, 768);
*
*      /// register channel 0 callback function
*      hx_drv_adcc_register_cb(ADCC_CH0_CB, adcc_callback_ch_0);
*
*      /// unmask channel 0 interrupt
*      hx_drv_adcc_set_int_mask(~(ADCC_CH1_HIGH_INT_BIT | ADCC_CH1_LOW_INT_BIT |ADCC_CH0_HIGH_OUT_INT_BIT | ADCC_CH0_HIGH_IN_INT_BIT | ADCC_CH0_LOW_OUT_INT_BIT | ADCC_CH0_LOW_IN_INT_BIT); 
*
*      /// unmask channel 0
*      hx_drv_adcc_set_channel_mask(~ADCC_CH0); 
*
 ...
 */
ADCC_ERROR_E hx_drv_adcc_init(uint32_t base_addr, ADCC_CFG_T *adcc_cfg);
 ...
 */
ADCC_ERROR_E hx_drv_adcc_set_sample_rate(ADCC_CHANNEL ADCC_CHANNEL , uint16_t sample_rate);
 ...
 * 
 *     data_type = (data_type<<ADCC_CH0_DATA_TYPE_BIT_SFT) | (data_type<<ADCC_CH1_DATA_TYPE_BIT_SFT) | \
 *                 (data_type<<ADCC_CH2_DATA_TYPE_BIT_SFT) | (data_type<<ADCC_CH3_DATA_TYPE_BIT_SFT);
 * 
 ...
 */
ADCC_ERROR_E hx_drv_adcc_set_data_type(ADCC_DATA_TYPE data_type);
 ...
 * ADCC_ERROR_INVALID_PARAMETERS, or ADCC_NO_ERROR.
 */
ADCC_ERROR_E hx_drv_adcc_set_data_offset(ADCC_CHANNEL  channel, uint16_t offset);
 ...
 * ADCC_ERROR_INVALID_PARAMETERS, or ADCC_NO_ERROR.
 */
ADCC_ERROR_E hx_drv_adcc_set_high_threshold(ADCC_CHANNEL  channel, uint16_t high_threshold);
 ...
 * ADCC_ERROR_INVALID_PARAMETERS, or ADCC_NO_ERROR.
 */
ADCC_ERROR_E hx_drv_adcc_set_low_threshold(ADCC_CHANNEL  channel, uint16_t low_threshold);
 ...
 * 
 *     data_select = (data_select<<ADCC_CH0_DATA_SEL_BIT_SFT) | (data_select<<ADCC_CH1_DATA_SEL_BIT_SFT) | \
 *                   (data_select<<ADCC_CH2_DATA_SEL_BIT_SFT) | (data_select<<ADCC_CH3_DATA_SEL_BIT_SFT);
 * 
 * @return an ADCC_ERROR_E enum value, which could be ADCC_ERROR_NOT_INIT if the ADCC module is not
 * initialized or ADCC_NO_ERROR if the data select is set successfully.
 */
ADCC_ERROR_E hx_drv_adcc_set_data_select(ADCC_DATA_SEL data_select);


/**
 * This function gets the sample data from a specified ADC channel and applies a gain and offset
 * correction if previously calibrated.
 * 
 * @param channel The ADC channel number to read data from. It can be ADCC_CHANNEL0, ADCC_CHANNEL1,
 * ADCC_CHANNEL2, or ADCC_CHANNEL3.
 * @param channel_data A pointer to a uint16_t variable where the sampled data (12bit) from the specified ADC
 * channel will be stored.
 * 
 * @return an ADCC_ERROR_E enum value, which can be one of the following: ADCC_ERROR_NOT_INIT,
 * ADCC_ERROR_INVALID_PARAMETERS, or ADCC_NO_ERROR.
 */
ADCC_ERROR_E hx_drv_adcc_get_sample_data(ADCC_CHANNEL channel, uint16_t *channel_data);
 ...
 * 
 * @param cb_type The type of ADCC callback function to register.
 * @param cb_func cb_func is a function pointer to the callback function that will be registered for
 * the specified cb_type. This function will be called when the corresponding ADCC interrupt occurs.
 * 
 * @return an ADCC_ERROR_E enum value, which could be ADCC_NO_ERROR or one of several error codes.
 */
